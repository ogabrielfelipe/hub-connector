...
export class InMemoryRoutingExecutionRepository
    ...
    update(routingExecution: RoutingExecution): Promise<RoutingExecution> {
        const index = this.routingExecution.findIndex((item) => item.getId() === routingExecution.getId());
        if (index === -1) {
    ...
    findById(id: string): Promise<RoutingExecution | null> {
        return Promise.resolve(this.routingExecution.find((item) => item.getId() === id) || null);
    }
    findAllByRoutingId({ routingId, page, limit }: { routingId: string; page?: number; limit?: number; }): Promise<{ docs: RoutingExecution[]; total: number; page: number; limit: number; }> {
        const docs = this.routingExecution.filter((item) => item.getRoutingId() === routingId);

...
describe("FindAllGatewayUseCase", () => {
    ...
    it("should find all gateways", async () => {
        ...
        expect(result.docs).toEqual([{ id: gateway.getId(), name: gateway.getName(), active: gateway.getActive() }]);
    });
    ...
    it('should find all gateways with page and limit', async () => {
        ...
        expect(result.docs).toEqual([{ id: gateway1.getId(), name: gateway1.getName(), active: gateway1.getActive() }]);
        expect(result.total).toEqual(1);

        const result2 = await useCase.execute({ page: 2, limit: 1 });
        expect(result2.docs).toEqual([{ id: gateway2.getId(), name: gateway2.getName(), active: gateway2.getActive() }]);

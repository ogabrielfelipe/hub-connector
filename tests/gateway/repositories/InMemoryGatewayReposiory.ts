...
export class InMemoryGatewayReposiory implements IGatewayRepository {
    ...
    public async findByKey(key: string): Promise<Gateway | null> {
        return this.gateways.find(gateway => gateway.getXApiKey() === key) || null;
    }

    public async findAll({ name, active, page = 1, limit = 10 }: { name?: string, active?: boolean, page?: number, limit?: number }): Promise<{ docs: GatewayDTO[], total: number, page: number, limit: number }> {
        let filteredGateways = this.gateways;
        if (name) {
            filteredGateways = filteredGateways.filter(gateway => gateway.getName() === name);
        } else if (active) {
            filteredGateways = filteredGateways.filter(gateway => gateway.getActive() === active);
        }
    ...
    public async update(gateway: Gateway): Promise<Gateway> {
        const index = this.gateways.findIndex(gateway => gateway.getId() === gateway.getId()) || 0;

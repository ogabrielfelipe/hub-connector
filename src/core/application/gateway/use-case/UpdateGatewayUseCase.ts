...
export class UpdateGatewayUseCase {
    ...
    async execute(command: UpdateGatewayCaseCommand): Promise<Gateway> {
        ...
        if (!ability.can(Actions.Update, "Gateway")) {
            this.logger.warn(`User ${command.currentUserId} does not have permission to update gateway ${command.gatewayId}`);
        ...
        if (command.routes) {
            ...
            if (gateway.getRoutes()?.length === 0) {
                ...
                const newRoutes = routes.filter(route => !existingRoutes?.some(existingRoute => existingRoute.path === route.path));

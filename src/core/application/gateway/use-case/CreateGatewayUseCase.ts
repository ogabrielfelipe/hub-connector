...
export class CreateGatewayUseCase {
    ...
    constructor(repository: IGatewayRepository, userRepository: IUserRepository, abilityFactory: CaslAbilityFactory, logger: ILogger) {
    ...
    async execute(command: CreateGatewayCaseCommand): Promise<Gateway> {
        ...
        if (!ability.can(Actions.Create, "Gateway")) {
            this.logger.warn(`User ${currentUser.getUsername()} does not have permission to create a new gateway`);
            throw new NotPermissionError();
        }


        const xApiKey = Buffer.from(randomBytes(64).toString('base64')).toString('base64');
        ...
        const gateway = Gateway.createNew(command.name, xApiKey, true, new Date(), new Date(), routes);

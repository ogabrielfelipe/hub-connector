...
import {
  ...
  FindOneGatewaySchema,
  UpdateGatewayParamsSchema,
  UpdateGatewaySchema,
...
export class GatewayController {
  ...
  constructor(
    ...
    this.createGatewayUseCase = new CreateGatewayUseCase(
      gatewayRepository,
      userRepository,
      caslAbilityFactory,
      logger,
    );
    this.findOneGatewayUseCase = new FindOneGatewayUseCase(
      gatewayRepository,
      userRepository,
      caslAbilityFactory,
      logger,
    ...
    this.updateGatewayUseCase = new UpdateGatewayUseCase(
      gatewayRepository,
      userRepository,
      caslAbilityFactory,
      logger,
    );
    this.deleteGatewayUseCase = new DeleteGatewayUseCase(
      gatewayRepository,
      userRepository,
      caslAbilityFactory,
      logger,
  ...
  async findOneGateway(req: FastifyRequest, reply: FastifyReply) {
    ...
    const result = await this.findOneGatewayUseCase.execute({
      currentUserId: currentUser!.userId,
  ...
  async deleteGateway(req: FastifyRequest, reply: FastifyReply) {
    ...
    const result = await this.deleteGatewayUseCase.execute({
      currentUserId: currentUser!.userId,
      gatewayId: gatewayId.gatewayId,
    });

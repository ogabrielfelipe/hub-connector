...
export class JWTService implements ITokenService {
  ...
  generate(payload: object) {
    return jwt.sign(payload, Buffer.from(this.PRIVATE_KEY, "base64"), { expiresIn: "1h", algorithm: "RS256" });
  }
  verify(token: string) {
    return jwt.verify(token, Buffer.from(this.PUBLIC_KEY, "base64")) as {userId: string; role: string};
  }
}

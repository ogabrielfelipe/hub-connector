...
import { IRoutingRepository, RoutingDetail, RoutingList, RoutingWithPagination } from "@/core/domain/routing/repositories/IRoutingRepository";
...
export class MongoRoutingRepository implements IRoutingRepository {
    ...
    async findAll({
        ...
        const resultFiltered: RoutingList[] = await Promise.all(routingModels.map(async (routingModel) => {
            const gateway = await this.gatewayRepository.findById(routingModel.gatewayId);
            if (!gateway) {

...
export class RoutingExecutionConverter {
  public toDomain(
    routingExecution: RoutingExecutionDocument,
  ): RoutingExecution {
    ...
    const logStatus: LogStatusType[] = routingExecution.logStatus.map(
      (log) => ({
  ...
  ): Partial<RoutingExecutionDocument> {
    // Convert logStatus from domain types to database documents
    const logStatus: LogStatusDocument[] = routingExecution
      .getLogStatus()
      .map((log) => ({

...
export class AuthController {
  ...
  constructor(
    ...
    this.loginUserWithGitHubSSOUseCase = new LoginUserWithGitHubSSOUseCase(userRepository, token);
  ...
  public async githubLogin(req: FastifyRequest, reply: FastifyReply) {
    const result = await this.app.githubOAuth2.generateAuthorizationUri(req, reply);
  ...
  public async githubCallback(req: FastifyRequest, reply: FastifyReply) {
    ...
    if (!primaryEmail) {
      return reply.status(400).send({ error: 'Nenhum email verificado encontrado no GitHub' })
    }

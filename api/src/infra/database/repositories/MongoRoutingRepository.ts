...
export class MongoRoutingRepository implements IRoutingRepository {
  ...
  async save(routing: Routing): Promise<Routing> {
    ...
    await this.cacheRepository.set(
      `routings:ById:${routingModel._id!.toString()}`,
      routingModel,
    );
  ...
  async delete(routing: Routing): Promise<void> {
    ...
    await Promise.all([
      ...
      this.cacheRepository.del(`routings:ById:${routingModel._id!.toString()}`),
      this.cacheRepository.del(
        `routings:byGatewayId:${routingModel.gatewayId}`,
      ),
      this.cacheRepository.del(`routings:bySlug:${routingModel.slug}`),
    ]);
  }
  ...
  async findOneDetail(id: string): Promise<RoutingDetail | null> {
    ...
    const gatewayModel = this.gatewayConverter.toPersistence(gateway);

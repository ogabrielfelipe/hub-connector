...
export class DashReportUseCase {
  ...
  async execute() {
    const totalRoutes = await this.routingRepository.findAll({
      page: this.DEFAULT_PAGE,
      limit: this.DEFAULT_PER_PAGE,
    });
    const totalConnectorExecutions =
      await this.repositoryExecutionSearch.search({
    ...
    const trafficPerRoute = totalConnectorExecutions.items.reduce(
      (acc, item) => {
        const route = totalRoutes.docs.find(
          (route) => route.id === item.routingId,
        );
    ...
    const latency_average =
      totalConnectorExecutions.items.reduce(
